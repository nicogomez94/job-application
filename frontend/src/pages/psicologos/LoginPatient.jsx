...
import { authService, patientAuthService } from '../../services';
...
export default function LoginPatient() {
  ...
  const submitRecovery = async (event) => {
    ...
    try {
      await authService.requestPasswordRecovery({ email: trimmedEmail, userType: 'patient' });
      ...
      <div className="psico-login-card">
        ...
        {isRecoveryModalOpen && (
          <div className="login-recovery-backdrop" onClick={closeRecoveryModal} role="presentation">
            <div className="login-recovery-modal" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="patient-recovery-title">
              <button type="button" className="login-recovery-close-btn" onClick={closeRecoveryModal} aria-label="Cerrar">
                &times;
              </button>
              <h3 id="patient-recovery-title" className="login-recovery-title">Recuperar clave</h3>
              ...
              <form className="login-recovery-form" onSubmit={submitRecovery}>
                <label className="login-label" htmlFor="patient-recovery-email">Email</label>
                <input
